...
constexpr uint8_t EXPECTED_VAR_COUNT = 6;
...
// IN3 (GPIO22): Right motor reverse
// IN4 (GPIO23): Right motor forward
constexpr int IN1 = 11;
constexpr int IN2 = 10;
constexpr int IN3 = 22;
constexpr int IN4 = 23;

constexpr int OUT1 = 18;
constexpr int OUT2 = 19;
constexpr int OUT3 = 20;
constexpr int OUT4 = 21;

constexpr int INP1 = 2;
constexpr int INP2 = 3;
...
void applyMotorSide(int16_t speed, int pinForward, int pinReverse) {
  ...
  if (clamped > 0) { //forward
    //forward on low of square wave on pinForward
    analogWrite(pinForward, 255 - duty);
    digitalWrite(pinReverse, HIGH);
  } 
  
  else if (clamped < 0) { //reverse
    //reverse on high of square wave on pinForward
    ...
    digitalWrite(pinReverse, LOW); //when both same, motor off
  }
}

int32_t clampOutput(int32_t value) {
  if (value < 0) return 0;
  if (value > 255) return 255;
  return value;
}

int32_t readInput8bit(int pin) {
  int raw = analogRead(pin);
  int32_t scaled = (int32_t)((raw * 255L + 2047) / 4095);
...
void applyAuxOutputs() {
  analogWrite(OUT1, (uint8_t)clampOutput(out1Value));
  analogWrite(OUT2, (uint8_t)clampOutput(out2Value));
  analogWrite(OUT3, (uint8_t)clampOutput(out3Value));
  analogWrite(OUT4, (uint8_t)clampOutput(out4Value));
}
...
void sendRegister() {
  ...
  auto addReadable = [&](const char* name) {
  ...
  // Telemetry is intentionally minimal for debugging WebSocket stability.
...
void handleMessage(const String& message) {
  ...
  if (strcmp(type, "target_write_update") == 0) {
    ...
    if (!values.isNull()) {
      if (values.containsKey("out1")) out1Value = (int32_t)values["out1"];
      if (values.containsKey("out2")) out2Value = (int32_t)values["out2"];
      if (values.containsKey("out3")) out3Value = (int32_t)values["out3"];
      if (values.containsKey("out4")) out4Value = (int32_t)values["out4"];
      applyAuxOutputs();
    }
...
void handleUdpPacket(int packetSize) {
  ...
  out1Value = clampOutput((int32_t)readInt16LE(buffer, 16));
  out2Value = clampOutput((int32_t)readInt16LE(buffer, 18));
  out3Value = clampOutput((int32_t)readInt16LE(buffer, 20));
  out4Value = clampOutput((int32_t)readInt16LE(buffer, 22));
...
void printStatus() {
  ...
  Serial.print(" | ");
  Serial.print(out1Value);
  Serial.print(" | ");
  Serial.print(out2Value);
  Serial.print(" | ");
  Serial.print(out3Value);
  Serial.print(" | ");
  Serial.print(out4Value);
  Serial.print(" | ");
  Serial.print(inp1Value);
  Serial.print(" | ");
  Serial.print(inp2Value);
  Serial.println();
}

void setup() {
  Serial.begin(115200);
  Serial.println("CODE VERSION: 9.3 - extra GPIO");
  Serial.print("[");
  Serial.print(millis());
...
void loop() {
  ...
  if (wsConnected && now - lastTelemetryMs >= TELEMETRY_INTERVAL_MS) {
    lastTelemetryMs = now;
    inp1Value = readInput8bit(INP1);
    inp2Value = readInput8bit(INP2);
    StaticJsonDocument<256> doc;
    doc["type"] = "target_telemetry";
    doc["target_id"] = targetId;
    JsonObject values = doc.createNestedObject("values");
    values["inp1"] = inp1Value;
    values["inp2"] = inp2Value;
    String payload;
    serializeJson(doc, payload);
    ws.send(payload);
  }

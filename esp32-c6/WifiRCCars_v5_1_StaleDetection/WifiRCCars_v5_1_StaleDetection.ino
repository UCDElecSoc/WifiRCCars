...
// Control stream detection (backend sends ~15 Hz fixed-rate writes)
constexpr unsigned long CONTROL_STREAM_STALE_MS = 2000;
constexpr unsigned long WRITE_UPDATE_LOG_MS = 1000;

// Optional diagnostics for unusually long polling
constexpr unsigned long POLL_WARN_MS = 150;
constexpr unsigned long POLL_WARN_LOG_MS = 2000;
...
unsigned long lastWsMessageMs = 0;
unsigned long lastWriteUpdateMs = 0;
unsigned long lastWriteLogMs = 0;
unsigned long lastPollWarnMs = 0;
...
void logWriteUpdateReceived() {
  unsigned long now = millis();
  if (now - lastWriteLogMs < WRITE_UPDATE_LOG_MS) return;
  lastWriteLogMs = now;
  Serial.println("[control] target_write_update received");
}

void markControlStreamActive() {
  if (controlStreamActive) return;
  controlStreamActive = true;
  Serial.println("[control] stream active");
}

void markControlStreamStale() {
  if (!controlStreamActive) return;
  controlStreamActive = false;
  Serial.println("[control] stream stale");
}
...
void handleMessage(const String& message) {
  ...
  if (strcmp(type, "target_write_update") == 0) {
    ...
    if (!values.isNull()) {
      ...
      lastWriteUpdateMs = lastCommandMs;
      setStatus("running");
      applyMotors();
      markControlStreamActive();
      logWriteUpdateReceived();
    }
...
void resetWebSocketClient(const char* reason) {
  Serial.print("[ws] reset: ");
...
void checkControlStreamStale() {
  if (!wsConnected) return;
  if (!controlStreamActive) return;

  unsigned long now = millis();
  if (now - lastWriteUpdateMs > CONTROL_STREAM_STALE_MS) {
    setStatus("stream-stale");
    markControlStreamStale();
    resetWebSocketClient("stale-control-stream");
  }
}
...
void printStatus() {
  ...
  Serial.print(" | stream=");
  Serial.print(controlStreamActive ? "active" : "idle");
  Serial.print(" | led=");
...
void loop() {
  ...
  ws.poll();
  unsigned long pollMs = (micros() - pollStart) / 1000;
  if (pollMs > POLL_WARN_MS) {
    unsigned long now = millis();
    if (now - lastPollWarnMs >= POLL_WARN_LOG_MS) {
      lastPollWarnMs = now;
      Serial.print("[ws] poll slow: ");
      Serial.print(pollMs);
      Serial.println(" ms");
    }
  }
